...
Сервис может быть пока с дефолтным конфигом — его мы переписываем в следующем шаге.

## 6. Генерация UUID для пользователей

Для каждого устройства генерируем отдельный UUID
```
uuidgen
```
Пример:
...
```
Эти UUID потом подставляются в конфиг Xray и в VLESS-ссылки.
...
```
Опционально:
Насчет скрипта, можно сделайть автомотическое добавление пользователя в конфиг, мне честно говоря лень этим заниматься, так как юзером у меня не много, легче ручками.

Единственное что сделал это быструю генерацию Vless ключа, а после добавляю в конфиг id и имя пользовател. Опять же в моем случае мне не нужно ограничивать duration пользователя и аналогичные свойства, все остальные доработки уже на ваших плечах.
На текущий момент 3.12.25 сборка полностью рабочая, никаких пробел нет.

Скрипт

Создаем файл
```
nano /root/make-vless.sh
```
...
echo
echo "VLESS ссылка:"
echo "vless://$UUID@$DOMAIN:443?encryption=none&flow=&type=ws&host=$DOMAIN&path=%2Fws&security=tls&sni=$DOMAIN#$NAME"
```
Делаем файл исполняемым
```
chmod +x /root/make-vless.sh
```
А после можете использовать комманду ```./make-vless.sh``` для добавления нового пользователя, после лишь заходите в конфиг и добавляете клиента, не забывайте перезапускать xray после добавления.
